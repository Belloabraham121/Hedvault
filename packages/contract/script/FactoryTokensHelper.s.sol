...
contract FactoryTokensHelper is Script {
    ...
    function checkPrerequisites() external {
        ...
        if (!isApproved) {
            ...
            console.log("Run: cast send", rwaTokenFactory, "\"approveCreator(address)\" ", creator);
        } else {
        ...
        console.log("PreciousMetals:", factory.supportedAssetTypes("PreciousMetals"));
        ...
        if (isApproved && balance >= required && !isPaused) {
            ...
            console.log("[WARNING] Prerequisites not met. Please resolve issues above.");
        }
    }
    ...
    function checkDeploymentStatus() external {
        ...
        string[14] memory expectedSymbols = [
            "HVGOLD", "HVSILVER", "HVPLATINUM", "HVPALLADIUM",
            "HVAAPL", "HVMSFT", "HVTSLA", "HVAMZN", "HVGOOGL",
            "HVOIL", "HVGAS", "HVWHEAT",
            "HVUST10Y", "HVCORP"
        ];
        
        console.log("\n--- Expected Tokens Status ---");
        for (uint i = 0; i < creatorTokens.length && i < expectedSymbols.length; i++) {
            uint256 tokenId = creatorTokens[i];
            address tokenAddress = factory.tokenById(tokenId);
            
            if (tokenAddress != address(0)) {
                DataTypes.AssetInfo memory info = factory.getAssetInfo(tokenAddress);
        ...
        if (creatorTokens.length >= 14) {
            ...
            console.log("[WARNING] Only", creatorTokens.length, "/ 14 tokens deployed");
        }
    }
    ...
    function _createPreciousMetalsTokens() internal {
        // Gold
        _createToken(
            "HedVault Gold Token", "HVGOLD", "PreciousMetals",
            "LBMA Certified Vaults, London", 2000 * 1e18, 10000 * 1e18,
            100 * 1e18, "ipfs://QmGoldCertification123"
        );
        
        // Silver
        _createToken(
            "HedVault Silver Token", "HVSILVER", "PreciousMetals",
            "COMEX Certified Vaults, New York", 25 * 1e18, 100000 * 1e18,
            50 * 1e18, "ipfs://QmSilverCertification456"
        );
        
        // Platinum
        _createToken(
            "HedVault Platinum Token", "HVPLATINUM", "PreciousMetals",
            "LPPM Certified Vaults, Zurich", 1000 * 1e18, 5000 * 1e18,
            200 * 1e18, "ipfs://QmPlatinumCertification789"
        );
        
        // Palladium
        _createToken(
            "HedVault Palladium Token", "HVPALLADIUM", "PreciousMetals",
            "LPPM Certified Vaults, London", 1500 * 1e18, 3000 * 1e18,
            300 * 1e18, "ipfs://QmPalladiumCertification101"
        );
    }
    ...
    function _createStockTokens() internal {
        // Apple
        _createToken(
            "HedVault Apple Stock Token", "HVAAPL", "Bonds",
            "NASDAQ, United States", 175 * 1e18, 1000000 * 1e18,
            175 * 1e18, "ipfs://QmAppleStockCertification"
        );
        
        // Microsoft
        _createToken(
            "HedVault Microsoft Stock Token", "HVMSFT", "Bonds",
            "NASDAQ, United States", 350 * 1e18, 800000 * 1e18,
            350 * 1e18, "ipfs://QmMicrosoftStockCertification"
        );
        
        // Tesla
        _createToken(
            "HedVault Tesla Stock Token", "HVTSLA", "Bonds",
            "NASDAQ, United States", 200 * 1e18, 500000 * 1e18,
            200 * 1e18, "ipfs://QmTeslaStockCertification"
        );
        
        // Amazon
        _createToken(
            "HedVault Amazon Stock Token", "HVAMZN", "Bonds",
            "NASDAQ, United States", 140 * 1e18, 600000 * 1e18,
            140 * 1e18, "ipfs://QmAmazonStockCertification"
        );
        
        // Google
        _createToken(
            "HedVault Google Stock Token", "HVGOOGL", "Bonds",
            "NASDAQ, United States", 130 * 1e18, 700000 * 1e18,
            130 * 1e18, "ipfs://QmGoogleStockCertification"
        );
    }
    ...
    function _createCommodityTokens() internal {
        // Oil
        _createToken(
            "HedVault Crude Oil Token", "HVOIL", "Commodities",
            "WTI Crude Oil Futures, NYMEX", 75 * 1e18, 100000 * 1e18,
            75 * 1e18, "ipfs://QmOilCertification"
        );
        
        // Natural Gas
        _createToken(
            "HedVault Natural Gas Token", "HVGAS", "Commodities",
            "Henry Hub Natural Gas, NYMEX", 3 * 1e18, 1000000 * 1e18,
            30 * 1e18, "ipfs://QmGasCertification"
        );
        
        // Wheat
        _createToken(
            "HedVault Wheat Token", "HVWHEAT", "Commodities",
            "CBOT Wheat Futures, Chicago", 6 * 1e18, 500000 * 1e18,
            60 * 1e18, "ipfs://QmWheatCertification"
        );
    }
    ...
    function _createBondTokens() internal {
        // US Treasury
        _createToken(
            "HedVault US Treasury Bond Token", "HVUST10Y", "Bonds",
            "US Treasury, Washington DC", 1000 * 1e18, 100000 * 1e18,
            1000 * 1e18, "ipfs://QmTreasuryBondCertification"
        );
        
        // Corporate Bond
        _createToken(
            "HedVault Corporate Bond Token", "HVCORP", "Bonds",
            "Corporate Bond Market, NYSE", 1000 * 1e18, 50000 * 1e18,
            1000 * 1e18, "ipfs://QmCorporateBondCertification"
        );
    }
    ...
    function _createToken(
        ...
        address tokenAddress = factory.createRWAToken{value: TOKEN_CREATION_FEE}(
            metadata, name, symbol, totalSupply
        );
