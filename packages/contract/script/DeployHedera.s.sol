...
contract HedVaultHederaDeployScript is Script {
    ...
    address constant USDC_USD_FEED = 0xa24de01df22b63d23Ebc1882a5E3d4ec0d907bFB;
    
    // Hedera network addresses
    address constant WHBAR_ADDRESS = 0x0000000000000000000000000000000000000163; // WHBAR on Hedera
    address constant HTS_PRECOMPILE = 0x0000000000000000000000000000000000000167; // Hedera Token Service
    ...
    function getHederaDeployConfig() internal view returns (DeployConfig memory) {
        return DeployConfig({
            feeRecipient: vm.envOr("FEE_RECIPIENT", msg.sender),
            rewardTokenSupply: vm.envOr("REWARD_TOKEN_SUPPLY", uint256(1000000000 * 1e18)),
            ...
            configureChainlinkOracles: vm.envOr("CONFIGURE_CHAINLINK", true),
            hederaConfig: HederaConfig({
    ...
    function deployContracts(DeployConfig memory config) internal {
        ...
        console.log("Deploying RWATokenFactory with Hedera Token Service integration...");
        deployed.rwaTokenFactory = address(new RWATokenFactory(deployed.hedVaultCore));
        console.log("RWATokenFactory deployed at:", deployed.rwaTokenFactory);
        
        // 8. Deploy SwapEngine
        console.log("Deploying SwapEngine...");
        deployed.swapEngine = address(new SwapEngine(
            deployed.hedVaultCore,
            deployed.priceOracle,
            config.feeRecipient
        ));
        ...
        deployed.complianceManager = address(new ComplianceManager(
            deployed.hedVaultCore,
            config.feeRecipient
        ));
        console.log("ComplianceManager deployed at:", deployed.complianceManager);
        
        // 10. Deploy PortfolioManager
        console.log("Deploying PortfolioManager...");
        deployed.portfolioManager = address(new PortfolioManager(
            deployed.hedVaultCore,
            deployed.priceOracle
        ));
        console.log("PortfolioManager deployed at:", deployed.portfolioManager);
        
        // 11. Deploy CrossChainBridge
        console.log("Deploying CrossChainBridge...");
        deployed.crossChainBridge = address(new CrossChainBridge(config.feeRecipient));
        console.log("CrossChainBridge deployed at:", deployed.crossChainBridge);
        
        // 12. Deploy verification contract
        ...
        // 13. Deploy mock tokens if requested
    ...
    function deployHederaMockTokens() internal {
        ...
        string[5] memory tokenNames = ["Wrapped HBAR", "USD Coin", "Wrapped Ether", "Wrapped Bitcoin", "Chainlink Token"];
        string[5] memory tokenSymbols = ["WHBAR", "USDC", "WETH", "WBTC", "LINK"];
    ...
    function configureChainlinkOracles(DeployConfig memory config) internal {
        ...
        if (config.chainlinkConfig.hbarUsdFeed != address(0)) {
            ...
            console.log("HBAR/USD price feed configured:", config.chainlinkConfig.hbarUsdFeed);
        }
    ...
    function logHederaDeploymentResults() internal view {
        ...
        if (deployed.mockTokens.length > 0) {
            ...
            string[5] memory symbols = ["WHBAR", "USDC", "WETH", "WBTC", "LINK"];
            for (uint i = 0; i < deployed.mockTokens.length && i < symbols.length; i++) {
                console.log(string(abi.encodePacked(symbols[i], ":")), deployed.mockTokens[i]);
            }
        }
        ...
        console.log("\n=== Deployment Summary ===");
        console.log("Total contracts deployed:", 11 + deployed.mockTokens.length);
